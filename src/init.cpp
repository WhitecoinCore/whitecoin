...
void Shutdown()
{
    ...
    RenameThread("whitecoin-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: whitecoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: whitecoind.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 15814 or testnet: 24070)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 15815 or testnet: 24071)") + "\n";
    ...
    strUsage += "  -loadblock=<file>      " + _("Imports blocks from external XWC000?.dat file") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Whitecoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Whitecoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
