    ...
    transactionReport(0)
{
    ...
    transactionReportT = viewT;
    transactionRreportT2 = viewT2;
...
void TransactionReport::setModel(WalletModel *model)
{
    this->model = model;
    if(model)
    {
        ...
        transactionReport->setModel(reportModel);
        transactionReport->setAlternatingRowColors(true);
        transactionReport->setSelectionBehavior(QAbstractItemView::SelectRows);
        transactionReport->setSelectionMode(QAbstractItemView::SingleSelection);
        transactionReport->setEditTriggers(QAbstractItemView::NoEditTriggers);
        transactionReport->setSortingEnabled(true);
        transactionReport->sortByColumn(0, Qt::DescendingOrder);
        transactionReport->verticalHeader()->hide();
        transactionReport->setShowGrid(false);        
        	
        reportModelT = new QStandardItemModel(this);
        transactionReportT->setModel(reportModelT);
        transactionReportT->setAlternatingRowColors(true);
        transactionReportT->setSelectionBehavior(QAbstractItemView::SelectRows);
        transactionReportT->setSelectionMode(QAbstractItemView::SingleSelection);
        transactionReportT->setEditTriggers(QAbstractItemView::NoEditTriggers);
        transactionReportT->verticalHeader()->hide();
        transactionReportT->setShowGrid(false); 
        
        reportModelT2 = new QStandardItemModel(this);
        transactionRreportT2->setModel(reportModelT2);
        transactionRreportT2->setAlternatingRowColors(true);
        transactionRreportT2->setSelectionBehavior(QAbstractItemView::SelectRows);
        transactionRreportT2->setSelectionMode(QAbstractItemView::SingleSelection);
        transactionRreportT2->setEditTriggers(QAbstractItemView::NoEditTriggers);
        transactionRreportT2->verticalHeader()->hide();
        transactionRreportT2->setShowGrid(false); 
        
        connect(transactionReport->selectionModel(), SIGNAL(selectionChanged(const QItemSelection &, const QItemSelection &)), this, SLOT(showTotal()));         
        showTotal();
    }
}

void TransactionReport::chooseDate(int idx)
{
...
void TransactionReport::chooseType(int idx)
{
    if(!transactionProxyModel)
        return;
...
void TransactionReport::changedPrefix(const QString &prefix)
{
    if(!transactionProxyModel)
        return;
...
void TransactionReport::changedAmount(const QString &amount)
{
    if(!transactionProxyModel)
        return;
...
void TransactionReport::exportClicked()
{
...
void TransactionReport::showTotal()
{
		if(!model)
			 return;
	  ...
	  {
	  	if (transactionProxyModel->data(transactionProxyModel->index(i,0),TransactionTableModel::StatusRole).toInt() == 0)
	  	{
			  	//ColumnIndex
			  	iTotal = transactionProxyModel->data(transactionProxyModel->index(i,4),TransactionTableModel::AmountRole).toLongLong();
			  	fTotal += iTotal;
			  	iTimes++;
		 	
			  	addressname=transactionProxyModel->data(transactionProxyModel->index(i,3)).toString();
			  	typelistname=transactionProxyModel->data(transactionProxyModel->index(i,2)).toString();
    ...
    transactionReport->horizontalHeader()->resizeSection(0, 270);
    transactionReport->horizontalHeader()->resizeSection(1, 120);
    transactionReport->horizontalHeader()->resizeSection(2, 120);
    transactionReport->horizontalHeader()->resizeSection(3, 140);
    transactionReport->horizontalHeader()->resizeSection(4, 140);
    ...
    { 
    	...
    	QString amountText = BitcoinUnits::format(unit, totallist.at(i), false);
    ...
    transactionRreportT2->horizontalHeader()->resizeSection(0, 270);
    transactionRreportT2->horizontalHeader()->resizeSection(1, 120);
    transactionRreportT2->horizontalHeader()->resizeSection(2, 120);
    transactionRreportT2->horizontalHeader()->resizeSection(3, 140);
    transactionRreportT2->horizontalHeader()->resizeSection(4, 140);
    ...
    { 
    	...
    	QString amountText = BitcoinUnits::format(unit, totallist2.at(i), false);
    ...
    transactionRreportT2->horizontalHeader()->hide();
    ...
    transactionReportT->horizontalHeader()->resizeSection(0, 270);
    transactionReportT->horizontalHeader()->resizeSection(1, 120);
    transactionReportT->horizontalHeader()->resizeSection(2, 120);
    transactionReportT->horizontalHeader()->resizeSection(3, 140);
    transactionReportT->horizontalHeader()->resizeSection(4, 140);
...
QWidget *TransactionReport::createDateRangeWidget()
{
...
void TransactionReport::dateRangeChanged()
{
    if(!transactionProxyModel)
        return;
    transactionProxyModel->setDateRange(
...
void TransactionReport::focusTransaction(const QModelIndex &idx)
{
    if(!transactionProxyModel)
        return;
    QModelIndex targetIdx = transactionProxyModel->mapFromSource(idx);
    transactionReport->scrollTo(targetIdx);
    transactionReport->setCurrentIndex(targetIdx);
    transactionReport->setFocus();
}
